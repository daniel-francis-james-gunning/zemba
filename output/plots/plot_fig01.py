...
# calculate horizontal ocean transport (surface layer)
...
# horizontal velocity (in x10^5 m/yr)
u1_velocity = u1 *(60*60*24*365) * 1e-5

# horizonal transport (in sverdrup)
u1_transport = u1*2*np.pi*Var["r_earth"]*0.7*np.cos(Var["olatbr"])*Var["odepth"][0] * 1e-6

# vertical velocity (in m/yr)
ww_velocity = ww *(60*60*24*365)

# vertical transport (in sverdrup)
ww_transport = ww * 2*np.pi*Var["r_earth"]*0.7*np.cos(Var["olatr"])*Var["r_earth"]*Var["dlatr"] * 1e-6
...
for i in np.arange(0,1):
    ...
    axs[i].format(
                  ylim = (0.3, 1),
                  ylocator = np.arange(0.3,1+0.1, 0.1),
                  yminorlocator = [],)
...
for i in np.arange(1,2):
    ...
    axs[i].format(
                  ylim = (-0.05, 1.05),
                  ylocator = np.arange(0,1+0.1, 0.2),
                  yminorlocator = np.arange(0,1+0.1, 0.1),)
...
for i in np.arange(3,4):
    ...
    axs[i].format(
                  ylim = (-4, 6),
                  ylocator = np.arange(-4,6+2, 2),
                  yminorlocator = np.arange(-4,6+1, 1),)
...
for i in np.arange(2,3):
    ...
    axs[i].format(
                  ylim = (-90, 20),
                  ylocator = np.arange(-80,20+20, 20),
                  yminorlocator = np.arange(-80,20+10, 10),)
...
axs11 = axs[1,1].twinx()
axs11.format(ylim = (-20, 25),
             ylabel = "Transport (Sv)",
             ylocator = np.arange(-20, 25+5,5),
             yminorlocator=[],
             ticklabelsize=8, ticklabelweight='normal', 
             ylabelsize=8, ylabelweight='normal',
             xlabelsize=8, xlabelweight='normal', 
             titlesize=8, titleweight='normal',)
axs11.yaxis.label.set_color('gray7')        #setting up X-axis label color to yellow
axs11.tick_params(axis='y', colors='gray7')    #setting up X-axis tick color to red
...
# align_yaxis(axs[1,0], 0, axs10, 0)

fig.save(os.getcwd()+"/output/plots/f01.png", dpi = 400)
fig.save(os.getcwd()+"/output/plots/f01.pdf", dpi = 400)