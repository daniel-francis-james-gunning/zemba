...
def precipitation(lat, ta, qa, surface_height, Lv, r, atm_rho, atm_depth, secs_in_day, 
    ...
    # precipitation flux
    precipitation_flux[i] = (atm_rho * atm_depth) / secs_in_day * (qa[i] - ( (r/100) * qsa[i]) )
    ...
    return precipitation_flux, precipitation_rate, precipitation_heat_flux
...
@njit(nogil = True, cache=True)
def snowfall(lat, tas, precip_flux, height, lr, Lm):
    ...
    return snowfall_fraction, snowfall_flux, snowfall_heat_flux



@njit(nogil = True, cache=True)
def snow_accumulation(snowfall_fraction, snow_fraction, snow_thick, precip_rate, 
                      sarea, secs_in_day, snow):
   ...
   if snow == 2:
       ...
       i = np.where(snowfall_fraction > snow_fraction0)
       ...
       j = np.where(snowfall_fraction < snow_fraction0)
...
def snow_melt(lat, snow_thick, snow_fraction, ts, lr, height, K, land_hc, Lm, 
    ...
    if snow == 2:
        ...
        # new snow thickness
        snow_thick1 = snow_thick0 - ( (melt_rate/2) ) 
        
        # new snow fraction
        snow_fraction1 = (snow_fraction0) - ( ( (melt_rate/2) * snow_fraction0 ) / snow_thick1)
...
float_array_1d = types.float64[:,:]

@njit(nogil = True, cache=True)
def ebm(Var, State, INPUT, I, znth_dw, settings, 
    ...
    StateYear = Dict.empty(key_type=types.unicode_type, value_type=float_array_1d)
    ...
    # list of variables to save for every model year...
    ...
    StateAnnual = Dict.empty(key_type=types.unicode_type, value_type=float_array_1d)
   
    for keys in key_variables:
        StateAnnual[keys] = np.zeros(( State[keys].shape[0], INPUT['nyrs'].astype('i8')[0]), dtype="f8") 
        ...
        for day in np.arange(0, 365): # for every day
            ...
            if fixed_land_albedo is None: # albedo calculated internally
                ...
                if snow == 0: # as a function of surface temperature
                    ...
                    State["alpha_land"] = surface_albedo_land_v2(Var["lat"], INPUT["land_height"], Var["lr"], INPUT["alphag"], State["Tal"], State["Tl"], INPUT["alphas"], INPUT["alphaws"], INPUT["alphai"], znth_dw[:,day], Var["land_mask"], INPUT['ice_fraction'])
                
                elif snow != 0: # as a function of snowfal/melt
                
                    State["alpha_land"] = surface_albedo_land(Var["lat"], State["snow_thick"], State["melt"], INPUT["alphag"], INPUT["alphai"], INPUT["alphas"], INPUT["alphaws"], INPUT['ice_fraction'], State["snow_fraction_land"], znth_dw[:,day], Var["land_mask"], snow)
            ...
            if (hydro == 1): # if hydrological cycle is on.
                ...
                State["precip_flux_land"], State["precip_rate_land"], State["lhf_precip_land"] = precipitation(Var["lat"], State["Tal"], State["Q_land"], INPUT["land_height"], Var["Lv"], INPUT["r"], Var["atm_rho"], Var["atm_depth"], Var["secs_in_day"], Var["land_mask"], Var["water_rho"], height)
                
                # Over ocean
                #-----------
                
                State["precip_flux_ocean"], State["precip_rate_ocean"], State["lhf_precip_ocean"] = precipitation(Var["lat"], State["Tao"], State["Q_ocean"], Var["ocean_height"], Var["Lv"], INPUT["r"], Var["atm_rho"], Var["atm_depth"], Var["secs_in_day"], Var["ocean_mask"], Var["water_rho"], height)
            ...
            if ( (hydro == 1) & (snow != 0) ): # if hydrological cycle is on.
                ...
                State["snowfall_fraction_land"], State["snowfall_flux_land"], State["lhf_snowfall_land"] = snowfall(Var["lat"], State["Tal"], State["precip_flux_land"], INPUT["land_height"], Var["lr"], Var["Lm"])
                
                # Over ocean
                #-----------
                
                State["snowfall_fraction_ocean"], State["snowfall_flux_ocean"], State["lhf_snowfall_ocean"] = snowfall(Var["lat"], State["Tao"], State["precip_flux_ocean"], Var["ocean_height"], Var["lr"], Var["Lm"])
            ...
            if fixed_snow_fraction is None and fixed_snow_thick is None and fixed_snow_melt is None: # snow is calculated internally...
                ...
                if ( (hydro == 1) & (snow != 0) ): # if hydrological cycle is on.
                    ...
                    State["snow_fraction_land"], State["snow_thick"] = snow_accumulation(State["snowfall_fraction_land"], State["snow_fraction_land"], State["snow_thick"], State["precip_rate_land"], Var["sarea"], Var["secs_in_day"], snow)
            ...
            if transport==1: # if transport is on.
                ...
                if year == INPUT['nyrs'].astype('i8')[0] - 1: # if last year of model run.
                    ...
                    if version == 0: # if classic 'dry' version of EBM 
                        ...
                        State["atm_north"]         = np.zeros((Var["latb"].size))
                        State["atm_conv"]          = np.zeros((Var["lat"].size))
                ...
                for n in np.arange(0, small_step): 
                    ...
                    if atm_transport==1: # if atmospheric heat transport turned on.
                        ...
                        if version == 0: # if classic 'dry' version of EBM 
                            ...
                            atm_north, atm_conv = dry_heat_transport(State['Ta'], np.zeros((Var["lat"].size)), Var['dlatr'], Var['swidth'], Var['atm_hc'], INPUT['dt'], Var['r_earth'], Var['sarea'])
                            
                            # atmospheric heating/cooling
                            State["Ta"] = State["Ta"] + (Var["secs_in_day"]/small_step) / Var["atm_hc"]  * (atm_conv)
                            
                        if version == 1: # if moist version of EBM   
                    
                            mse_north, mse_conv, hadley_latent, hadley_dry, eddy_latent, eddy_dry, dry_north, dry_conv, latent_north, latent_conv = moist_heat_transport_v2(Var, State, INPUT, hadley_cell, hydro, height)
                    ...
                    if year == INPUT['nyrs'].astype('i8')[0] - 1: # if last year of model run.
                        ...
                        if atm_transport==1: # if atmospheric heat transport turned on.
                            ...
                            if version == 0: # if classic 'dry' version of EBM 
                                ...
                                State["atm_north"]  = State["atm_north"] + atm_north
                                State["atm_conv"]   = State["atm_conv"] + atm_conv
                ...
                if year == INPUT['nyrs'].astype('i8')[0] - 1: # if last year of model run.
                    ...
                    if atm_transport==1: # if atmospheric heat transport turned on.
                        ...
                        if version == 0: # if classic 'dry' version of EBM 
                            ...
                            State["atm_north"]         = State["atm_north"]        /  small_step
                            State["atm_conv"]          = State["atm_conv"]         /  small_step
            ...
            State['Tlx']  = State["Tlx"]  - (Var["lr"] * INPUT["land_height"])
            ...
            for keys in key_variables:
                ...
                if day == 364: # if last day of year
                
                    StateAnnual[keys][:,year] = mean_numba(StateYear[keys])[0]
