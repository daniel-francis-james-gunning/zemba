...
def get_constants(INPUT):
    ...
    # no. of days                                       
    Var["ndays"] = np.linspace(0., 365.2422, 365) 
    ...
    Var["atm_depth"] = np.array([5000.]) # (m)
    
    # Specific heat capacity of air 
    Var["atm_sphc"] = np.array([1000.]) # (J Kg^-1 K^-1)
    
    # Air Density 
    Var["atm_rho"] = np.array([1.5]) # (Kg m^-3)
    ...
    # Sea ice thickness 
    Var["si_ithick"] = np.zeros((Var["lat"].size)) + 2. # (m)
...
def initialize_state(Var, INPUT):
    ...
    State["rsds_land"] = np.copy(State["rsut_land"]) # Downwards shortwave radiation at surface 
    State["rsus_land"] = State["Tl"].copy()          # Upwards shortwave radiation at surface 
    State["rsdtnet_land"] = State["Tl"].copy()       # Net downwards shortwave radiation at TOA
    State["rsdsnet_land"] = State["Tl"].copy()       # Net downwards shortwave radiation at surface
    State["rsatmnet_land"] = State["Tl"].copy()      # Absorbed shortwave radiation by atmosphere
    

    # Over ocean
    #-----------
    State["rsut_ocean"] = State["Tl"].copy()     # Upwards shortwave radiation at TOA 
    State["rsds_ocean"] = State["Tl"].copy()     # Downwards shortwave radiation at surface 
    State["rsus_ocean"] = State["Tl"].copy()     # Upwards shortwave radiation at surface 
    State["rsdtnet_ocean"] = State["Tl"].copy()  # Net downwards shortwave radiation at TOA
    State["rsdsnet_ocean"] = State["Tl"].copy()  # Net downwards shortwave radiation at surface
    State["rsatmnet_ocean"] = State["Tl"].copy() # Absorbed shortwave radiation by atmosphere
    
    
    # Weighted Average
    #-----------------
    State["rsut"] = State["Tl"].copy()      # Upwards shortwave radiation at TOA 
    State["rsds"] = State["Tl"].copy()      # Downwards shortwave radiation at surface 
    State["rsus"] = State["Tl"].copy()      # Upwards shortwave radiation at surface 
    State["rsdtnet"] = State["Tl"].copy()   # Net downwards shortwave radiation at TOA
    State["rsdsnet"] = State["Tl"].copy()   # Net downwards shortwave radiation at surface
    State["rsatmnet"] = State["Tl"].copy()  # Absorbed shortwave radiation by atmosphere
    ...
    # Over land
    #----------
    State["rlus_land"] = State["Tl"].copy()      # Upwards longwave radiation at surface
    State["rlds_land"] = State["Tl"].copy()      # Downwards longwave radiation at surface 
    State["rlut_land"] = State["Tl"].copy()      # Upwards longwave radiation at TOA 
    State["rldsnet_land"] = State["Tl"].copy()   # Net downwards longwave radiation at surface
    State["rlatmnet_land"] = State["Tl"].copy()  # Absorbed longwave radiation by atmosphere

    # Over ocean
    #-----------
    State["rlus_ocean"] = State["Tl"].copy()      # Upwards longwave radiation at surface 
    State["rlds_ocean"] = State["Tl"].copy()      # Downwards longwave radiation at surface 
    State["rlut_ocean"] = State["Tl"].copy()      # Upwards longwave radiation at TOA 
    State["rldsnet_ocean"] = State["Tl"].copy()   # Net downwards longwave radiation at surface
    State["rlatmnet_ocean"] = State["Tl"].copy()  # Absorbed longwave radiation by atmosphere


    # Weighted average
    #-----------------
    State["rlus"] = State["Tl"].copy()      # Upwards longwave radiation at surface
    State["rlds"] = State["Tl"].copy()      # Downwards longwave radiation at surface 
    State["rlut"] = State["Tl"].copy()      # Upwards longwave radiation at TOA 
    State["rldsnet"] = State["Tl"].copy()   # Net downwards longwave radiation at surface
    State["rlatmnet"] = State["Tl"].copy()  # Absorbed longwave radiation by atmosphere
    ...
    # Over land
    #----------
    State["rtnet_land"]   = State["Tl"].copy() # Net downwards radiation at TOA
    State["rsnet_land"]   = State["Tl"].copy() # Net downwards radiation at surface
    State["ratmnet_land"] = State["Tl"].copy() # Radiation absorbed by atmosphere
    
    # Over ocean
    #-----------
    State["rtnet_ocean"]   = State["Tl"].copy() # Net downwards radiation at TOA
    State["rsnet_ocean"]   = State["Tl"].copy() # Net downwards radiation at surface
    State["ratmnet_ocean"] = State["Tl"].copy() # Radiation absorbed by atmosphere
    
    # Weighted average
    #-----------------
    
    State["rtnet"]   = State["Tl"].copy()                      # Net downwards radiation at TOA
    State["rsnet"]   = State["Tl"].copy()                      # Net downwards radiation at surface
    State["ratmnet"] = State["Tl"].copy( )                     # Radiation absorbed by atmosphere
    State["rtimb"] = np.array([global_mean(State["Ta"], Var)]) # Radiation imbalance 
    ...
    State["si_volume"][idx[0]]   = Var["oarea"][i[0]] * State["si_thick"][i[0]]
