...
input_zemba['res']=np.array([1.])  # <----------------------------------------------------------- HERE
...
# LGM from ICE6G_C dataset (Argus et al., 2014; Peltier et al., 2014)
# ice6g_lat = pd.read_csv(script_path+'/other_data/ice6g/ice6g.txt', skiprows=7, usecols=[0], sep='\t').to_numpy()[:,0]
# ice6g_if  = pd.read_csv(script_path+'/other_data/ice6g/ice6g.txt', skiprows=7, usecols=[7], sep='\t').to_numpy()[:,0]
# input_zemba['ice_fraction'] = np.interp(lat, ice6g_lat, ice6g_lf)
